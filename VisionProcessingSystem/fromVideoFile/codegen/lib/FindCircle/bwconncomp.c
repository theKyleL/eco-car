 ...
 * Return Type  : void
 */
void bwconncomp(const boolean_T varargin_1[307200], double *CC_Connectivity,
                ...
                *CC_RegionIndices, emxArray_int32_T *CC_RegionLengths)
{
  ...
  numRuns = 0;
  for (firstRunOnPreviousColumn = 0; firstRunOnPreviousColumn < 640;
       firstRunOnPreviousColumn++) {
    lastRunOnPreviousColumn = firstRunOnPreviousColumn * 480;
  ...
  if (numRuns == 0) {
    ...
    *CC_Connectivity = 8.0;
    for (lastRunOnPreviousColumn = 0; lastRunOnPreviousColumn < 2;
      ...
      CC_ImageSize[lastRunOnPreviousColumn] = 480.0 + 160.0 * (double)
        lastRunOnPreviousColumn;
    }
    ...
    *CC_NumObjects = 0.0;
    ...
    for (firstRunOnPreviousColumn = 0; firstRunOnPreviousColumn < 640;
         firstRunOnPreviousColumn++) {
      lastRunOnPreviousColumn = firstRunOnPreviousColumn * 480 - 1;
      nextLabel = 1;
      while (nextLabel <= 480) {
        while ((nextLabel <= 480) && (!varargin_1[nextLabel +
                lastRunOnPreviousColumn])) {
          nextLabel++;
        }

        if ((nextLabel <= 480) && varargin_1[nextLabel + lastRunOnPreviousColumn])
        {
    ...
    emxFree_int32_T(&x);
    for (k = 0; k + 1 <= numRuns; k++) {
      runCounter = (startCol->data[k] - 1) * 480;
