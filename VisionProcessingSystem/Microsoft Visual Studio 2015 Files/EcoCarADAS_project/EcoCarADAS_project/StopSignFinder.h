...
class StopSignFinder: public IFinder
{
public:
	StopSignFinder();
	~StopSignFinder();
	...
	{
		return (char*)&stopSignData;
	};
	virtual int getStructSize()
	{
		return sizeof(StopSignData);
	};
...
private:
	...
	//Stop sign memory
	int SSMemSize = 10;
	cv::Rect stopSignMemFrame1[10];
	cv::Rect stopSignMemFrame2[10];
	int confidenceFrame1 = 0;
	int confidenceFrame2 = 0;

	//Transmit data
	StopSignData stopSignData;
	int confidence = 0; // The average confidence percentage that there is a stop sign in view
	cv::Rect stopSignToTransmit; // The location of the stop sign in the right frame (frame1) because that is the one being displayed as of now.
	double distanceToSS = -1;

	int frameConfidenceToTransmit = 10; // If the average confidence is above 10% transmit data
	bool foundStopSign = false;
