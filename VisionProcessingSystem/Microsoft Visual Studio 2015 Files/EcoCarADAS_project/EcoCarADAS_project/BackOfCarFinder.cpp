...
void BackOfCarFinder::find(cv::Mat* frame1, cv::Mat* frame2, unsigned int frameSeqNum)
{
	...
	backOfCarData.header.sequence = frameSeqNum;
	//speedLimitSignData.header.type = speed_limit_sign; //////////////////////////////////////?????????????????????????????????????????? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ?

	//Launch Thread
	finderThread = std::thread(&BackOfCarFinder::processFrame1and2, this);
}

void BackOfCarFinder::processFrame1and2()
{
	std::thread thread1(&BackOfCarFinder::finder, this, 1);
	std::thread thread2(&BackOfCarFinder::finder, this, 2);
	thread1.join();
	thread2.join();

	bool tempTest = false;

	if ((backOfCarMemFrame1[0].x > -1) && (backOfCarMemFrame2[0].x > -1))
	{
		double deltaX = abs((backOfCarMemFrame1[0].x + (backOfCarMemFrame1[0].width / 2)) - (backOfCarMemFrame2[0].x + (backOfCarMemFrame2[0].width / 2)));

		distanceToBOC = DISTANCE_CALC_VALUE / deltaX;
		tempTest = true;
	}
	else
	{
	...
	backOfCarToTransmit = backOfCarMemFrame1[0];					// Only sending the location of the stop sign in the right frame because that is the one being shown on the display.
	...
	{
		foundBackOfCar = false;
	}

	backOfCarData.confidence = confidence;
	backOfCarData.distance = distanceToBOC;
	backOfCarData.x_location = (backOfCarToTransmit.x - (backOfCarToTransmit.width / 2));
	backOfCarData.y_location = (backOfCarToTransmit.y - (backOfCarToTransmit.height / 2));
	backOfCarData.radius = backOfCarToTransmit.width / 2;
	...
	std::cout << "SS Distance: " << distanceToBOC << "\n" << "SS Confidence: " << confidence << "\n\n";
}

void BackOfCarFinder::finder(int frameNum)
{
	...
	int BOCaverageX = 0;
	int BOCaverageY = 0;
	int BOCaverageWidth = 0;
	int BOCaverageHeight = 0;

	//Stop sign memory
	cv::Rect backOfCarMem[10];

	int backOfCarPercentage = 0;
	...
	if (frameNum == 1)
	{
		...
		for (int i = 0; i < BOCMemSize; i++)
		{
			backOfCarMem[i] = backOfCarMemFrame1[i];
		}
	}
	else if (frameNum == 2)
	{
		...
		for (int i = 0; i < BOCMemSize; i++)
		{
			backOfCarMem[i] = backOfCarMemFrame2[i];
		}
	}
	...
	for (int i = BOCMemSize - 1; i > 0; i--)
	{
		backOfCarMem[i] = backOfCarMem[i - 1];
	}
	...
	if (frameNum == 1)
	{
		...
		for (int i = 0; i < BOCMemSize; i++)
		{
			backOfCarMemFrame1[i] = backOfCarMem[i];
		}

		confidenceFrame1 = backOfCarPercentage;
	}
	else if (frameNum == 2)
	{
		...
		for (int i = 0; i < BOCMemSize; i++)
		{
			backOfCarMemFrame2[i] = backOfCarMem[i];
		}

		confidenceFrame2 = backOfCarPercentage;
	}
}


BackOfCarFinder::BackOfCarFinder()
{
	//////////////////////////////////////////////////////////////////////////////////////////
