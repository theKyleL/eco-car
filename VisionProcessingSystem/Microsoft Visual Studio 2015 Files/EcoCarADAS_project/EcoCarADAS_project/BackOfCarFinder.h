...
class BackOfCarFinder : public IFinder
{
public:
	BackOfCarFinder();
	~BackOfCarFinder();
...
private:
	...
	//Stop sign memory
	int BOCMemSize = 10;
	cv::Rect backOfCarMemFrame1[10];
	cv::Rect backOfCarMemFrame2[10];
	int confidenceFrame1 = 0;
	int confidenceFrame2 = 0;

	//Transmit data
	StopSignData backOfCarData;
	int confidence = 0; // The average confidence percentage that there is a stop sign in view
	cv::Rect backOfCarToTransmit; // The location of the stop sign in the right frame (frame1) because that is the one being displayed as of now.
	double distanceToBOC = -1;

	int frameConfidenceToTransmit = 10; // If the average confidence is above 10% transmit data
	bool foundBackOfCar = false;
