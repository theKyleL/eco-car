...
///
/// See confluence for most up to date and descriptive information on this stop sign finder.
/// http://db-atlassian.cloudapp.net/confluence/pages/viewpage.action?pageId=6164201
///
/// This code frame from the stearo video capture class and looks for stop signs in it.
/// It is able to find objects by reading in two xml files of trained data of stop signs
...
/// is looking at has been reduced in two ways.  The first way is by taking a small part
...
/// height and width. The contours that are found to be stop signs are also check to see if
/// there bounding rectangle is close to a square.
///
///
/// References
/// [1] 	C.Dahms, "GitHub MicrocontrollersAndMore,"[Online].Available: https://github.com/MicrocontrollersAndMore/OpenCV_3_KNN_Character_Recognition_Cpp. [Accessed November 2016].
/// [2] 	D.V.Jernej Mrovlje, "Distance measuring based on stereoscopic pictures," 9th International PhD Workshop on Systems and Control : Young Generation Viewpoint, 2008.
...
void StopSignFinder::find(cv::Mat* frame1, cv::Mat* frame2, unsigned int frameSeqNum)
{
	...
	stopSignData.header.sequence = frameSeqNum;
	stopSignData.header.type = stop_signs;

	//Launch Thread
	finderThread = std::thread(&StopSignFinder::processFrame1and2, this);
}

void StopSignFinder::processFrame1and2()
{
	if (CAMERA_2_ENABLED == 1)
	{
		std::thread thread1(&StopSignFinder::finder, this, 1);
		std::thread thread2(&StopSignFinder::finder, this, 2);
		...
		if ((stopSignMemFrame1[0].x > -1) && (stopSignMemFrame2[0].x > -1))
		{
			double deltaX = abs((stopSignMemFrame1[0].x + (stopSignMemFrame1[0].width / 2)) - (stopSignMemFrame2[0].x + (stopSignMemFrame2[0].width / 2)));

			distanceToSS = DISTANCE_CALC_VALUE / deltaX;
			tempTest = true;
		}
		else
		{
		...
		stopSignToTransmit = stopSignMemFrame1[0];					// Only sending the location of the stop sign in the right frame because that is the one being shown on the display.
		...
		stopSignData.confidence = confidence;
		stopSignData.distance = distanceToSS;
		stopSignData.x_location = (stopSignToTransmit.x - (stopSignToTransmit.width / 2));
		stopSignData.y_location = (stopSignToTransmit.y - (stopSignToTransmit.height / 2));
		stopSignData.radius = stopSignToTransmit.width / 2;
		...
		std::cout << "SS Distance: " << distanceToSS << "\n" << "SS Confidence: " << confidence << "\n\n";
	}
	else
	{
		std::thread thread1(&StopSignFinder::finder, this, 1);
		...
		stopSignData.confidence = confidence;
		stopSignData.distance = -1; //Unknown
		stopSignData.x_location = (stopSignToTransmit.x - (stopSignToTransmit.width / 2));
		stopSignData.y_location = (stopSignToTransmit.y - (stopSignToTransmit.height / 2));
		stopSignData.radius = stopSignToTransmit.width / 2;
...
void StopSignFinder::finder(int frameNum)
{
	...
	int SSaverageX = 0;
	int SSaverageY = 0;
	int SSaverageWidth = 0;
	int SSaverageHeight = 0;

	//Stop sign memory
	cv::Rect stopSignMem[10];
	...
	if (frameNum == 1)
	{
		...
		{
			stopSignMem[i] = stopSignMemFrame1[i];
		}
	}
	else if (frameNum == 2)
	{
		...
		for (int i = 0; i < SSMemSize; i++)
		{
			stopSignMem[i] = stopSignMemFrame2[i];
		}
	}
	...
	cv::cvtColor(matReducedBMDIT, matGrayscale, CV_BGR2GRAY);         
	...
	if (frameNum == 1)
	{
		...
		for (int i = 0; i < SSMemSize; i++)
		{
			stopSignMemFrame1[i] = stopSignMem[i];
		}

		confidenceFrame1 = stopSignPercentage;
	}
	else if (frameNum == 2)
	{
		...
		for (int i = 0; i < SSMemSize; i++)
		{
			stopSignMemFrame2[i] = stopSignMem[i];
		}

		confidenceFrame2 = stopSignPercentage;
	}
}


StopSignFinder::StopSignFinder()
{
	//////////////////////////////////////////////////////////////////////////////////////////
	...
	/// [1] end																								// even though in reality they are multiple images / numbers
