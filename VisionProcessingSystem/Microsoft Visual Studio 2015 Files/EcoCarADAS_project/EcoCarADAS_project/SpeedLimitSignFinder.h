...
class SpeedLimitSignFinder : public IFinder
{
public:
	SpeedLimitSignFinder();
	~SpeedLimitSignFinder();
...
private:
	...
	//Stop sign memory
	int SLSMemSize = 10;
	cv::Rect speedLimitSignMemFrame1[10];
	cv::Rect speedLimitSignMemFrame2[10];
	int confidenceFrame1 = 0;
	int confidenceFrame2 = 0;

	//Transmit data
	StopSignData speedLimitSignData;
	int confidence = 0; // The average confidence percentage that there is a stop sign in view
	cv::Rect speedLimitSignToTransmit; // The location of the stop sign in the right frame (frame1) because that is the one being displayed as of now.
	double distanceToSLS = -1;

	int frameConfidenceToTransmit = 10; // If the average confidence is above 10% transmit data
	bool foundSpeedLimitSign = false;
