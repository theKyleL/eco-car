...
#include "stdafx.h"
#include "SpeedLimitSignFinder.h"
...
void SpeedLimitSignFinder::find(cv::Mat* frame1, cv::Mat* frame2, unsigned int frameSeqNum)
{
	...
	speedLimitSignData.header.sequence = frameSeqNum;
	//speedLimitSignData.header.type = speed_limit_sign; //////////////////////////////////////?????????????????????????????????????????? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ?

	//Launch Thread
	finderThread = std::thread(&SpeedLimitSignFinder::processFrame1and2, this);
}

void SpeedLimitSignFinder::processFrame1and2()
{
	std::thread thread1(&SpeedLimitSignFinder::finder, this, 1);
	std::thread thread2(&SpeedLimitSignFinder::finder, this, 2);
	thread1.join();
	thread2.join();

	bool tempTest = false;

	if ((speedLimitSignMemFrame1[0].x > -1) && (speedLimitSignMemFrame2[0].x > -1))
	{
		double deltaX = abs((speedLimitSignMemFrame1[0].x + (speedLimitSignMemFrame1[0].width / 2)) - (speedLimitSignMemFrame2[0].x + (speedLimitSignMemFrame2[0].width / 2)));

		distanceToSLS = DISTANCE_CALC_VALUE / deltaX;
		tempTest = true;
	}
	else
	{
	...
	speedLimitSignToTransmit = speedLimitSignMemFrame1[0];					// Only sending the location of the stop sign in the right frame because that is the one being shown on the display.


	if (confidence >= frameConfidenceToTransmit)
	{
		foundSpeedLimitSign = true;
	}
	else
	{
		foundSpeedLimitSign = false;
	}

	speedLimitSignData.confidence = confidence;
	speedLimitSignData.distance = distanceToSLS;
	speedLimitSignData.x_location = (speedLimitSignToTransmit.x - (speedLimitSignToTransmit.width / 2));
	speedLimitSignData.y_location = (speedLimitSignToTransmit.y - (speedLimitSignToTransmit.height / 2));
	speedLimitSignData.radius = speedLimitSignToTransmit.width / 2;
	...
	std::cout << "SS Distance: " << distanceToSLS << "\n" << "SS Confidence: " << confidence << "\n\n";
}

void SpeedLimitSignFinder::finder(int frameNum)
{
	...
	int SLSaverageX = 0;
	int SLSaverageY = 0;
	int SLSaverageWidth = 0;
	int SLSaverageHeight = 0;

	//Stop sign memory
	cv::Rect speedLimitSignMem[10];

	int speedLimitSignPercentage = 0;
	...
	if (frameNum == 1)
	{
		...
		for (int i = 0; i < SLSMemSize; i++)
		{
			speedLimitSignMem[i] = speedLimitSignMemFrame1[i];
		}
	}
	else if (frameNum == 2)
	{
		...
		for (int i = 0; i < SLSMemSize; i++)
		{
			speedLimitSignMem[i] = speedLimitSignMemFrame2[i];
		}
	}
	...
	speedLimitSignPercentage = 0;

	//adds 8 percent for every stop sign that has been seen in the last 10 frames
	for (int i = 0; i < SLSMemSize; i++)
	{
		if (speedLimitSignMem[i].x > 0)
		{
			speedLimitSignPercentage += 8;
		}
	}
	...
	for (int i = SLSMemSize - 1; i > 0; i--)
	{
		speedLimitSignMem[i] = speedLimitSignMem[i - 1];
	}
	...
	if (frameNum == 1)
	{
		...
		for (int i = 0; i < SLSMemSize; i++)
		{
			speedLimitSignMemFrame1[i] = speedLimitSignMem[i];
		}

		confidenceFrame1 = speedLimitSignPercentage;
	}
	else if (frameNum == 2)
	{
		...
		for (int i = 0; i < SLSMemSize; i++)
		{
			speedLimitSignMemFrame2[i] = speedLimitSignMem[i];
		}

		confidenceFrame2 = speedLimitSignPercentage;
	}
}


SpeedLimitSignFinder::SpeedLimitSignFinder()
{
	//////////////////////////////////////////////////////////////////////////////////////////
