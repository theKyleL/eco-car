...
int main() {
	...
	imgTrainingNumbers = cv::imread("Training_Images4/parkingLot16.png");          // read in training numbers image
	...
	for (int i = 0; i < ptContours.size(); i++) {                           // for each contour
		...
		if (((boundingRect.width > MIN_CONTOUR_WIDTH) && (boundingRect.width < MAX_CONTOUR_WIDTH)
			...
			cv::rectangle(imgTrainingNumbers, boundingRect, cv::Scalar(0, 0, 0), 2); //cover up past rec
